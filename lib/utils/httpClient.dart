...
class ExtractorHttpClient {
  static const Map<String, String> defaultHeaders = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:68.0) Gecko/20100101 Firefox/68.0'};

  static Future<int> getContentLength(String url, {Map<String, String> headers}) async {
    var response = await http.head(Uri.parse(url), headers: headers ?? defaultHeaders);
    print("HEADERS CL: ${response.headers.toString()}");
    return int.tryParse(response.headers['content-length'] ?? '0');
  }

  static Stream<List<int>> getStream(dynamic stream, {Map<String, String> headers, bool validate = true, int start = 0, int errorCount = 0}) async* {
    ...
    for (var i = start; i < stream.size; i += 9898989) {
      try {
        ...
        yield* getStream(stream, headers: headers, validate: validate, start: bytesCount, errorCount: errorCount + 1);
        break;
      }
    }
    client.close();
